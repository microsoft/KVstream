...
def serve(
    ...
    backend_url:     Optional[str]  = typer.Option(None, "--backend-url",  help="Override backend base URL"),
    model:           Optional[str]  = typer.Option(None,                   help="Model name (backend default if omitted)"),
    port:            int            = typer.Option(8080,                   help="Proxy port"),
    host:            str            = typer.Option("127.0.0.1",            help="Bind address (use 0.0.0.0 to expose on the network — no auth layer!)"),
    ...
    config_file:     Optional[str]  = typer.Option(None, "--config",        help="Path to kvault.yaml"),
    ...
    from kvstream.backends import OllamaBackend, FoundryBackend, LlamaCppBackend, LMStudioBackend

    if backend not in _DEFAULT_URLS:
        console.print(f"[red]Unknown backend '{backend}'. Choose: {list(_DEFAULT_URLS)}")
        raise typer.Exit(1)

    # Load kvault.yaml (if present) for base URL / model defaults
    cfg = KVaultConfig.auto(config_file)
    yaml_url   = cfg.backend.base_url
    yaml_model = cfg.backend.model

    # CLI flag > kvault.yaml > hardcoded default
    ...
    cfg.observability.log_level     = log_level.upper()
...
def bench(
    ...
    async def _run():
        prompt = ("Tell me about large language model inference optimisation. " * 10)[:prompt_len * 6]
            ...
            async with sem:
                ...
                try:
                    async with httpx.AsyncClient(timeout=120) as c:
                        async with c.stream("POST", f"{url}/v1/chat/completions", json={
                            "model": model,
        ...
        with Progress(SpinnerColumn(), TextColumn("{task.description}"), TimeElapsedColumn()) as p:
            task = p.add_task(f"Running {total_requests} requests (concurrency={concurrency})",
        ...
        t.add_row("p99",         f"{latencies[min(int(n*.99),n-1)]*1000:.0f} ms")
        console.print(t)
...
def health(url: str = typer.Option("http://localhost:8080")):
    ...
    try:
        ...
        console.print(f"KVStream proxy : [{'green' if ok else 'yellow'}]{d.get('status','?')}[/]  ({url})")
        console.print(f"Backend   : [{'green' if be else 'red'}]{'healthy (ok)' if be else 'unreachable (!!)'}[/]  "
