...
Priority order (highest to lowest):
  ...
  3. kvstream.yaml in the working directory
  4. Built-in defaults
"""
...
class KVStreamConfig(BaseSettings):
    """
    ...
    @classmethod
    def from_yaml(cls, path: str) -> KVStreamConfig:
        """
    ...
    @classmethod
    def auto(cls, config_file: str | None = None) -> KVStreamConfig:
        """
        Load config automatically:
          - Use config_file if provided
          - Otherwise try kvstream.yaml (or legacy kvault.yaml) in cwd
        ...
        for candidate in ("kvstream.yaml", "kvault.yaml"):
